...
fn test_help_output() {
    ...
    assert!(
        !stdout.contains("\n  workflow"),
        "Top-level help must not advertise a workflow command, got: {stdout}"
    );
    assert!(
        stdout.contains("job"),
...
#[test]
fn test_workflow_subcommand_is_unavailable() {
    let output = oxo_call()
        .args(["workflow", "--help"])
        .output()
        .expect("failed to run oxo-call");
    assert!(
        !output.status.success(),
        "workflow must not be accepted as a top-level subcommand"
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("unrecognized subcommand") || stderr.contains("unknown"),
