use super::task_values::{TaskValues, get_known_subcommands_for_tool, rule_based_subcommand_match};
...
fn compute_word_overlap_score(
    task_words: &std::collections::HashSet<&str>,
    desc_lower: &str,
) -> i32 {
...
fn compute_semantic_bonus(
    ...
    if (desc_lower.contains("reference")
        || desc_lower.contains("ref genome")
    ...
    if (desc_lower.contains("database") || flag_lower.contains("db"))
        && (task_lower.contains("database")
            ...
            || task_lower.contains("blast"))
    {
        bonus += 8;
    }

    if (desc_lower.contains("annotation")
    ...
    let semantic_pairs: &[(&[&str], &[&str], i32)] = &[
        (
            &["quality", "qual"],
            &["quality", "qual", "trim", "filter"],
            8,
        ),
        (
            &["region", "chrom", "window"],
            &["region", "chrom", "window", "interval", "locus"],
            10,
        ),
        ...
        (
            &["bootstrap", "replicat"],
            &["bootstrap", "replicat", "resample"],
            15,
        ),
        (
            &["evalue", "e-value", "expect"],
            &["evalue", "e-value", "significance"],
            12,
        ),
        (
            &["identity", "similarity"],
            &["identity", "similarity", "percent", "match"],
            10,
        ),
        (&["coverage", "depth"], &["coverage", "depth", "cov"], 10),
        (
            &["preset"],
            &["preset", "sensitivity", "sensitive", "fast"],
            8,
        ),
        (
            &["method"],
            &["method", "algorithm", "approach", "mode", "strategy"],
            8,
        ),
        ...
        (
            &["cutoff", "threshold"],
            &["cutoff", "threshold", "minimum", "min", "maximum", "max"],
            10,
        ),
        (
            &["pvalue", "p-value", "pval"],
            &["pvalue", "p-value", "pval", "significance"],
            15,
        ),
        (&["format"], &["format", "output format", "type"], 8),
        (
            &["length", "len"],
            &["length", "len", "size", "bp", "base"],
            5,
        ),
        (
            &["adapter", "barcode", "index"],
            &["adapter", "barcode", "index", "ligation"],
            12,
        ),
        (
            &["contig", "scaffold"],
            &["contig", "scaffold", "assembly", "assemble"],
            10,
        ),
        (
            &["variant", "snp", "genotype"],
            &["variant", "snp", "genotype", "call", "mutation"],
            10,
        ),
        (
            &["expression", "abundance", "count"],
            &[
                "expression",
                "abundance",
                "count",
        ...
        (
            ...
            &["annotation", "feature", "gene", "transcript", "exon", "cds"],
            10,
        ),
        (
            &["taxonomic", "classify", "taxon"],
            &["taxonomic", "classify", "taxon", "taxonomy", "identify"],
            10,
        ),
        (
            &["read", "read1", "read2"],
            &["read", "read1", "read2", "mate", "pair-end", "single-end"],
            8,
        ),
        (
            &["bam", "sam", "alignment"],
            &["bam", "sam", "alignment", "mapped", "align"],
            8,
        ),
        (
            &["vcf", "variant", "genotype"],
            &["vcf", "variant", "genotype", "snp", "indel"],
            8,
        ),
        (
            &["fastq", "fq", "sequence"],
            &["fastq", "fq", "sequence", "read", "raw"],
            8,
        ),
        (
            &["bed", "interval", "peak"],
            &["bed", "interval", "peak", "region", "chip"],
            8,
        ),
        (
            &["fasta", "reference", "genome"],
            &["fasta", "reference", "genome", "assembly", "fna"],
            8,
        ),
        (
            &["gtf", "gff", "annotation"],
            &["gtf", "gff", "annotation", "transcript", "gene model"],
            8,
        ),
        (
            &["mem", "memory", "buffer"],
            &["mem", "memory", "buffer", "ram"],
            3,
        ),
        (
            &["compress", "decompress", "gzip", "zip"],
            &["compress", "decompress", "gzip", "zip", "gz", "archive"],
            8,
        ),
        (
            &["score", "bit", "bitscore"],
            &["score", "bit", "bitscore", "ranking"],
            8,
        ),
        (
            &["gap", "mismatch", "penalty"],
            &["gap", "mismatch", "penalty", "open", "extend"],
            8,
        ),
        (
            &["motif", "pattern", "domain"],
            &["motif", "pattern", "domain", "consensus", "pfam"],
            10,
        ),
        (
            &["tree", "phylogeny", "newick"],
            &["tree", "phylogeny", "newick", "phylogen", "branch"],
            10,
        ),
        (
            &["busco", "completeness", "lineage"],
            &["busco", "completeness", "lineage", "quality", "assessment"],
            10,
        ),
        (
            &["trim", "clip", "cut"],
            &["trim", "clip", "cut", "remove", "discard"],
            10,
        ),
        (
        ...
        (
            &["split", "partition", "chunk"],
            &["split", "partition", "chunk", "divide"],
            8,
        ),
        (
            &["sort", "order", "arrange"],
        ...
        (
            &["dedup", "duplicate", "markdup"],
            &["dedup", "duplicate", "markdup", "remove duplicate"],
            10,
        ),
        (
            &["realignment", "recalibration", "base recal"],
            &["realignment", "recalibration", "base recal", "bqsr"],
            10,
        ),
        (
            &["contamination", "cross"],
            &["contamination", "cross", "pollut"],
            10,
        ),
    ];
...
fn compute_negative_score(desc_lower: &str, flag_lower: &str, task_lower: &str) -> i32 {
    ...
    if desc_lower.contains("verbose") || desc_lower.contains("quiet") {
        neg -= 20;
    }
    if desc_lower.contains("debug") && !task_lower.contains("debug") {
        neg -= 20;
    }
    ...
    if desc_lower.contains("config")
        && !task_lower.contains("config")
        && !task_lower.contains("setting")
    {
        neg -= 3;
    }
    ...
    if desc_lower.contains("intermediate") {
        neg -= 8;
    }
    if desc_lower.contains("progress") && !task_lower.contains("progress") {
        neg -= 5;
    }
    if desc_lower.contains("statistics")
        && !task_lower.contains("stat")
    ...
    if desc_lower.contains("dry-run")
        || desc_lower.contains("dry_run")
        || desc_lower.contains("pretend")
    {
        neg -= 15;
    }
    if (desc_lower.contains("force") || desc_lower.contains("overwrite"))
        && !task_lower.contains("force")
        && !task_lower.contains("overwrite")
    {
        neg -= 3;
    }
    if desc_lower.contains("trace") && !task_lower.contains("trace") {
        neg -= 10;
    }
    if (desc_lower.contains("benchmark") || desc_lower.contains("timing"))
        && !task_lower.contains("benchmark")
    {
        neg -= 8;
    }
    if desc_lower.contains("citation") || desc_lower.contains("cite") {
        neg -= 10;
    }
    if desc_lower.contains("warranty") || desc_lower.contains("license") {
        neg -= 15;
    }
...
fn resolve_flag_value(
    ...
    if desc_lower.contains("output") || flag_lower.contains("out") {
            ...
            || flag_lower == "-n"
        {
            return task_values
                ...
                .or_else(|| {
                    infer_output_from_input_for_rule(
                        desc_lower,
                        flag_lower,
    ...
    if desc_lower.contains("input") || flag_lower.contains("in") {
        ...
        if desc_lower.contains("fasta")
            || desc_lower.contains("fna")
            || desc_lower.contains("genome")
        {
            return task_values
                .reference_files
                .iter()
        ...
        || flag_lower.contains("outfmt")
    {
        ...
        if task_lower.contains("bam") {
            return Some("bam".to_string());
        }
        if task_lower.contains("sam") {
            return Some("sam".to_string());
        }
        if task_lower.contains("vcf") {
            return Some("vcf".to_string());
        }
        if task_lower.contains("json") {
            return Some("json".to_string());
        }
        if task_lower.contains("tsv") {
            return Some("tsv".to_string());
        }
        if task_lower.contains("csv") {
            return Some("csv".to_string());
        }
        return entry.default.clone();
    }

    if desc_lower.contains("readgroup")
        || flag_lower.contains("read-group")
        || flag_lower.contains("rg")
    {
        if task_lower.contains("read group")
            || task_lower.contains("readgroup")
            || task_lower.contains("rg_")
        {
            let rg_match = task_lower
                .split("read group")
                .nth(1)
    ...
    if desc_lower.contains("platform") || flag_lower.contains("platform") {
        if task_lower.contains("illumina") {
            return Some("ILLUMINA".to_string());
        }
        if task_lower.contains("pacbio") {
            return Some("PACBIO".to_string());
        }
        if task_lower.contains("ont") || task_lower.contains("oxford") {
            return Some("ONT".to_string());
        }
        return entry.default.clone();
    }
    ...
    if (desc_lower.contains("preset") || flag_lower.contains("preset"))
        && !desc_lower.contains("format")
    {
        if task_lower.contains("very-sensitive") {
            return Some("very-sensitive".to_string());
        }
        if task_lower.contains("sensitive") {
            return Some("sensitive".to_string());
        }
        if task_lower.contains("very-fast") {
            return Some("very-fast".to_string());
        }
        if task_lower.contains("fast") && !task_lower.contains("fastq") {
            return Some("fast".to_string());
        }
        if task_lower.contains("pacbio") || task_lower.contains("hifi") {
            return Some("map-pb".to_string());
        }
        if task_lower.contains("ont") || task_lower.contains("nanopore") {
            return Some("map-ont".to_string());
        }
        if task_lower.contains("sr") || task_lower.contains("short-read") {
    ...
    if desc_lower.contains("min-length")
        || desc_lower.contains("min_len")
        || flag_lower.contains("minlen")
    {
        return task_values
            .numbers
            .iter()
    ...
    if desc_lower.contains("max-length")
        || desc_lower.contains("max_len")
        || flag_lower.contains("maxlen")
    {
        return task_values
            .numbers
            .iter()
    ...
    if desc_lower.contains("min-quality")
        || desc_lower.contains("min_qual")
        || flag_lower.contains("minqual")
    {
        return task_values
            .numbers
            .iter()
...
pub fn assemble_command_from_rules(
    ...
    let effective_subcommand = selected_subcommand
        .map(|s| s.to_string())
        .or_else(|| {
    ...
    let score_flag = |entry: &FlagEntry| -> i32 {
        ...
        if let Some(ref sub) = effective_subcommand {
            ...
            if desc_lower.contains(&sub_lower) {
                score += 5;
            }
        }
    ...
    for entry in &all_flags {
        ...
        if included_flags.contains(&flag_key) {
            continue;
        }
        if let Some(ref alt) = entry.alt_form {
            if included_flags.contains(alt) {
                continue;
            }
        }
        ...
        let is_thread_flag = desc_lower.contains("thread")
            || desc_lower.contains("cpu")
            || flag_lower == "-@"
            || flag_lower.contains("thread");
        let is_ref_flag = desc_lower.contains("reference")
            || flag_lower.contains("ref")
            || desc_lower.contains("genome")
            || flag_lower == "-x";
        let is_db_flag = desc_lower.contains("database") || flag_lower.contains("db");
        let is_annotation_flag = desc_lower.contains("annotation")
            || desc_lower.contains("gtf")
            || desc_lower.contains("gff");
        let is_quality_flag = desc_lower.contains("quality")
            || desc_lower.contains("qual")
            || flag_lower.contains("qual")
            || flag_lower.contains("minq")
            || desc_lower.contains("min-quality")
            || desc_lower.contains("mapping quality");
        let is_region_flag = desc_lower.contains("region")
            || desc_lower.contains("chrom")
            || desc_lower.contains("interval")
            || flag_lower.contains("region")
            || flag_lower == "-r" && desc_lower.contains("region");
        let is_sample_flag = desc_lower.contains("sample")
            || flag_lower.contains("sample")
            || desc_lower.contains("rg-id")
            || desc_lower.contains("read-group");
        let is_format_flag = (desc_lower.contains("format") || flag_lower.contains("format"))
            && !desc_lower.contains("stdout");
        let is_index_flag = desc_lower.contains("index")
            || flag_lower.contains("index")
            || desc_lower.contains("prefix") && desc_lower.contains("index");

        let should_auto_include = (is_output_flag
            && (task_has_output
                || task_values.input_files.iter().any(|f| {
                    let fl = f.to_ascii_lowercase();
                    fl.ends_with(".bam")
                        || fl.ends_with(".sam")
                        || fl.ends_with(".fastq")
            ...
            || (is_ref_flag
                && (!task_values.reference_files.is_empty()
                    || !task_values.genome_dirs.is_empty()))
            ...
            || (is_region_flag
                && (task_lower.contains("region") || task_lower.contains("chromosome")))
            || (is_sample_flag
                && (task_lower.contains("sample") || task_lower.contains("read group")))
            || (is_format_flag
                && (task_lower.contains("format")
                    || task_lower.contains("bam")
                    || task_lower.contains("vcf")))
    ...
    let positional_remaining: Vec<&String> = task_values
        .input_files
        .iter()
    ...
    if !task_values.numbers.is_empty() && !sdoc.has_subcommands {
        ...
        let has_number_arg = task_values
            .numbers
            .iter()
            .any(|n| args_str.contains(n.as_str()));
        if !has_number_arg {
