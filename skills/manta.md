...
## Concepts
- Manta uses a two-step workflow: (1) configManta to configure; (2) python runWorkflow.py -m local -j N to execute.
...
- The output directory (--runDir) is created by configManta and contains runWorkflow.py.
...
- --unstrandedRNA is for RNA-seq data without strand information.
...
### configure and run Manta germline SV calling (configManta)
**Args:** `configManta --bam sorted.bam --referenceFasta reference.fa --runDir manta_output && python manta_output/runWorkflow.py -m local -j 8`
**Explanation:** configure sets up runDir; execute with -m local and -j 8 threads; results in manta_output/results/variants/

### run Manta somatic SV calling for tumor-normal pair (configManta)
**Args:** `configManta --normalBam normal.bam --tumorBam tumor.bam --referenceFasta reference.fa --runDir manta_somatic && python manta_somatic/runWorkflow.py -m local -j 8`
**Explanation:** --normalBam and --tumorBam for somatic mode; outputs somaticSV.vcf.gz in results/variants/

### run Manta on WES data with capture regions (configManta)
**Args:** `configManta --bam sample.bam --referenceFasta reference.fa --exome --callRegions targets.bed.gz --runDir manta_wes && python manta_wes/runWorkflow.py -m local -j 8`
**Explanation:** --exome adjusts parameters for WES; --callRegions restricts to target regions (bgzipped + tabix-indexed BED)

### run Manta for RNA fusion detection (configManta)
**Args:** `configManta --rna --bam rna_sorted.bam --referenceFasta reference.fa --runDir manta_rna && python manta_rna/runWorkflow.py -m local -j 8`
...
**Args:** `configManta --bam sample.bam --referenceFasta reference.fa --generateEvidenceBam --runDir manta_evidence && python manta_evidence/runWorkflow.py -m local -j 8`
...
**Args:** `configManta --bam sample.bam --referenceFasta reference.fa --region chr1:1000000-2000000 --region chr2:5000000-6000000 --runDir manta_regions && python manta_regions/runWorkflow.py -m local -j 8`
...
**Args:** `configManta --bam sample.bam --referenceFasta reference.fa --outputContig --runDir manta_contigs && python manta_contigs/runWorkflow.py -m local -j 8`
...
**Args:** `configManta --tumorBam tumor.bam --referenceFasta reference.fa --runDir manta_tumor_only && python manta_tumor_only/runWorkflow.py -m local -j 8`
...
**Args:** `configManta --bam sample.bam --referenceFasta reference.fa --runDir manta_tuned && python manta_tuned/runWorkflow.py -m local -j 8 --memGb 32`
...
**Args:** `configManta --rna --unstrandedRNA --bam rna.bam --referenceFasta reference.fa --runDir manta_rna_unstranded && python manta_rna_unstranded/runWorkflow.py -m local -j 8`
**Explanation:** --unstrandedRNA for RNA-seq without strand information; required for certain library prep protocols

### run Manta germline analysis with multiple samples
**Args:** `configManta --bam sample1.bam --bam sample2.bam --bam sample3.bam --referenceFasta reference.fa --runDir manta_multi && python manta_multi/runWorkflow.py -m local -j 8`
