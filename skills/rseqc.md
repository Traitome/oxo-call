...
**Explanation:** -r provides a housekeeping gene BED file for unbiased coverage estimation; detects 5' or 3' coverage bias indicating RNA degradation or library prep issues; outputs .txt, .pdf, and .r files

### calculate FPKM counts with FPKM_count.py
**Args:** `FPKM_count.py -i sorted.bam -r hg38.bed -o fpkm_counts`
**Explanation:** calculates raw fragment count, FPM, and FPKM per gene; useful for expression-level QC and cross-sample comparison before differential analysis; outputs tab-delimited text file

### convert BAM to BigWig with bam2wig.py
**Args:** `bam2wig.py -i sorted.bam -s hg38.chrom.sizes -o output -u`
**Explanation:** -s provides chromosome sizes file from UCSC; -u skips multi-mapping reads; automatically converts to BigWig if wigToBigWig is in PATH; useful for genome browser visualization
