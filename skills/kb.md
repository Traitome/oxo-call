...
- Main command: kb count for processing FASTQ to count matrix; kb ref for reference building.
- Use -x to specify technology: 10xv3 (10x Chromium v3), 10xv2, 10xv1, INDROPS, CELSEQ, CELSEQ2.
- kb count takes barcode+UMI reads as first argument and cDNA reads second: -1 R1.fastq.gz -2 R2.fastq.gz.
...
- Use --h5ad to output AnnData format directly for Scanpy.
- kb ref --workflow lamanno builds a reference with both spliced and unspliced sequences.

## Pitfalls

- The -x technology flag must match the library preparation protocol exactly.
- For 10x v3: -1 is R1 (barcode+UMI, 28 bp), -2 is R2 (cDNA) — check file naming.
- kb uses kallisto internally — ensure kallisto is installed and in PATH.
- The reference must be built with kb ref matching the desired workflow (standard vs lamanno vs nac).
