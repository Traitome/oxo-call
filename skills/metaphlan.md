...
- Use --db_dir and --index to specify database path and index prefix (--db_dir is deprecated).
...
- For large datasets, save the mapping output (--mapout) to avoid re-aligning when re-running (--mapout is deprecated).
...
- --db_dir is deprecated; use --db_dir instead for specifying database location.
...
**Args:** `--input_type fastq --db_dir /path/to/mpa_db --index mpa_vJan21_CHOCOPhlAnSGB_202103 --nproc 8 reads.fastq.gz -o sample_profile.txt`
**Explanation:** --input_type fastq; --db_dir database directory; -o output profile table

### profile paired-end metagenomic reads
**Args:** `--input_type fastq --db_dir /path/to/mpa_db --index mpa_vJan21_CHOCOPhlAnSGB_202103 --nproc 8 -o sample_profile.txt R1.fastq.gz,R2.fastq.gz`
...
**Args:** `--input_type fastq --db_dir /path/to/mpa_db --index mpa_vJan21_CHOCOPhlAnSGB_202103 --nproc 8 --mapout sample.bowtie2.bz2 -o sample_profile.txt reads.fastq.gz`
**Explanation:** --mapout saves alignment for re-use; avoids re-aligning when re-running with different parameters
