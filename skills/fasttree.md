...
**Explanation:** -pseudo uses pseudocounts for highly gapped sequences; recommended for sparse alignments to improve numerical stability
...
**Explanation:** -constraints specifies alignment with 0/1/- defining splits; tree search tries to satisfy constraints (may not be fully met)

### infer tree without support values for faster execution
**Args:** `-nt -gtr -nosupport aligned_sequences.fasta > nosupport_tree.nwk`
**Explanation:** -nosupport disables support value computation; significantly faster when supports are not needed for downstream analysis

### infer tree with detailed log file
**Args:** `-nt -gtr -log tree_log.txt aligned_sequences.fasta > tree.nwk`
**Explanation:** -log saves intermediate trees, model parameters, and settings to tree_log.txt; useful for troubleshooting and reproducibility
...
**Explanation:** -nome -mllen optimizes branch lengths without changing topology; -intree provides starting tree; useful for refining approximate trees

### infer tree with custom SPR settings
**Args:** `-nt -gtr -spr -sprlength 15 -mlnni 4 aligned_sequences.fasta > spr_tree.nwk`
**Explanation:** -spr enables SPR moves; -sprlength 15 allows longer moves; -mlnni 4 increases ML NNI rounds; more thorough search for better accuracy
