...
**Explanation:** angsd command; -bam BAM list file; -GL 1 SAMtools likelihood model; -doGlf 2 outputs BEAGLE format for downstream PCAngsd; -doMaf 1 with -doMajorMinor 1 infers alleles from GL; -SNP_pval 1e-6 filters to likely variable sites; -minMapQ 30 minimum mapping quality; -minQ 20 minimum base quality; -nThreads 16 parallel processing; -out output prefix
...
**Explanation:** angsd command; -bam BAM list file; -GL 1 SAMtools likelihood model; -doSaf 1 computes per-site allele frequency likelihoods; -anc provides ancestral allele state for unfolded SFS; -minMapQ 30 minimum mapping quality; -minQ 20 minimum base quality; -nThreads 16 parallel processing; -out pop1 output prefix; outputs pop1.saf.idx

### estimate 1D site frequency spectrum from doSaf output
**Args:** `realSFS pop1.saf.idx -P 16 > pop1.sfs`
**Explanation:** realSFS companion tool; uses EM algorithm to estimate the SFS from .saf.idx files; -P 16 sets parallel threads; output is a space-separated SFS vector
...
**Explanation:** realSFS companion tool; two-step: compute 2D SFS with realSFS (-P 16 threads), then index Fst (-sfs uses 2D SFS, -fstout output prefix); use `realSFS fst stats pop1_pop2.fst.idx` for genome-wide Fst
...
**Explanation:** angsd command; -bam BAM list file; -GL 1 SAMtools likelihood model; -doAsso 2 uses Score Test; -doMaf 1 allele frequency estimation; -doMajorMinor 1 infers alleles from GL; -y supplies phenotype file (one value per individual); -minMapQ 30 minimum mapping quality; -minQ 20 minimum base quality; -nThreads 16 parallel processing; -out association output prefix; outputs association statistics

### generate consensus FASTA from BAM alignment
**Args:** `-i sample.bam -GL 1 -doFasta 2 -doCounts 1 -minMapQ 30 -minQ 20 -out consensus`
**Explanation:** angsd command; -i sample.bam single BAM input; -GL 1 SAMtools likelihood model; -doFasta 2 uses the most common base at each position; -doCounts 1 required for base counting; -minMapQ 30 minimum mapping quality; -minQ 20 minimum base quality; -out consensus output prefix; outputs .fa
...
**Explanation:** angsd command; -bam BAM list file; -GL 1 SAMtools likelihood model; -doGlf 2 outputs BEAGLE format needed by NGSadmix; -doMajorMinor 1 infers alleles from GL; -doMaf 1 allele frequency estimation; -SNP_pval 1e-6 filters to likely variable sites; -minMapQ 30 minimum mapping quality; -minQ 20 minimum base quality; -minInd 40 requires data in at least 40 individuals per site; -nThreads 16 parallel processing; -out admix_input output prefix; then run `NGSadmix -beagle admix_input.beagle.gz -K 3 -o admix_result`
