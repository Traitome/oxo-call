...
**Explanation:** fastq_screen command; --conf fastq_screen.conf points to the database config file; --outdir results/ output directory; --threads 8 parallel alignment; sample_R1.fastq.gz input file
...
**Explanation:** fastq_screen command; --conf fastq_screen.conf database config file; --subset 0 disables subsampling and screens every read; --outdir results/ output directory; --threads 8 parallel threads; sample_R1.fastq.gz input file; slower but detects rare contaminants
...
**Explanation:** fastq_screen command; --conf fastq_screen.conf database config file; --aligner bismark for bisulfite-treated libraries; --paired enables paired-end mode; --outdir results/ output directory; --threads 8 parallel threads; R1.fastq.gz R2.fastq.gz input files
...
**Explanation:** fastq_screen command; --conf fastq_screen.conf database config file; --no_html suppresses HTML/PNG generation; --outdir results/ output directory; --threads 8 parallel threads; sample.fastq.gz input file; faster when only the TXT table is needed
...
**Explanation:** fastq_screen command; --conf custom_screen.conf custom config should include a DATABASE line for Mycoplasma bowtie2 index; --outdir results/ output directory; --threads 8 parallel threads; sample_R1.fastq.gz input file; same command, custom config
...
**Explanation:** fastq_screen command; --conf fastq_screen.conf database config file; --nohits extracts reads unmapped to all genomes; --outdir results/ output directory; --threads 8 parallel threads; sample.fastq.gz input file; useful for identifying novel sequences or contamination
...
**Explanation:** fastq_screen command; --conf fastq_screen.conf database config file; --filter 1000 keeps reads uniquely mapping to first genome (1=unique, 0=unmapped for others); --outdir results/ output directory; --threads 8 parallel threads; sample.fastq.gz input file; removes human contamination
...
**Explanation:** fastq_screen command; --conf fastq_screen.conf database config file; --tag adds alignment codes to read headers; --subset 0 processes all reads; --outdir results/ output directory; --threads 8 parallel threads; sample.fastq.gz input file; output can be used for downstream filtering
...
**Explanation:** fastq_screen command; --conf fastq_screen.conf database config file; --filter 1000 filter code; --pass 1 means read passes if it maps to at least 1 genome; --outdir results/ output directory; --threads 8 parallel threads; sample.fastq.gz input file; acts as OR operator for multi-genome screening
...
**Explanation:** fastq_screen command; --conf fastq_screen.conf database config file; --filter 1000 filter code; --inverse inverts filter; --outdir results/ output directory; --threads 8 parallel threads; sample.fastq.gz input file; keeps reads NOT mapping to first genome; useful for removing known contaminants

### download pre-indexed reference genomes
**Args:** `--get_genomes`
**Explanation:** fastq_screen command; --get_genomes flag; downloads pre-indexed Bowtie2 genomes for common species; run once to set up screening databases
...
**Explanation:** fastq_screen command; --conf bisulfite_screen.conf database config file; --bisulfite uses Bismark aligner for bisulfite libraries; --paired paired-end mode; --outdir results/ output directory; --threads 8 parallel threads; R1.fastq.gz R2.fastq.gz input files; requires Bismark-indexed databases in config
