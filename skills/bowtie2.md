...
- bowtie2 index building: use '-build ref.fa index_prefix' to create the index; this invokes bowtie2-build and produces reference_index.*.bt2 files.
...
- The index build step ('-build') must complete before any alignment step ('-x') can run.

## Examples

### build a bowtie2 index from a reference FASTA
**Args:** `-build reference.fa reference_index`
**Explanation:** -build invokes the index builder; creates reference_index.*.bt2 files used by all subsequent alignments
...
### build index for a large reference genome in multi-threaded mode
**Args:** `-build --threads 8 reference.fa reference_index`
**Explanation:** --threads N speeds up index building for large genomes; creates reference_index.*.bt2 index files
