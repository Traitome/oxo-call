...
- annotatePeaks maps peaks to genomic features (promoter, exon, intron, intergenic) using a GTF/GFF annotation; it also computes motif enrichment at peaks.
- findMotifsGenome performs de novo and known motif enrichment analysis; it requires a genome FASTA or a HOMER genome package (e.g., hg38).
...
- -size parameter in findMotifsGenome controls the window around peak centers: 50bp for primary TF motifs, 200bp for co-enriched motifs, 1000bp for histone regions.
...
- getDifferentialPeaksReplicates performs differential peak analysis with replicates using DESeq2.
- analyzeChIP-Seq automates the full ChIP-seq pipeline from tag directory to motif analysis.

## Pitfalls

- HOMER is a multi-binary suite. Each tool is a separate command: findPeaks, makeTagDirectory, annotatePeaks, findMotifsGenome, mergePeaks, pos2bed, makeUCSCfile, configureHomer. ARGS for each tool start with their own flags — there is NO single 'homer' subcommand pattern.
...
- findMotifsGenome requires write access to the output directory; it creates many intermediate files and fails silently on permission errors.
- annotatePeaks outputs a large table to stdout by default — always redirect to a file or use -o.
...
- HOMER genome packages must be pre-installed (perl configureHomer -install hg38); specifying an uninstalled genome silently uses wrong sizes.
- Peak files must have unique IDs in the first column; duplicate IDs cause crashes or incorrect results; use renamePeaks if needed.
- Peak files saved from Excel on Mac must be in "Text (Windows)" format; use checkPeakFile to verify format.
- Long motif lengths (>12bp) with large -size values can cause excessive memory usage and long runtimes.
- findMotifsGenome -mask is recommended to avoid repeat regions confounding motif discovery.
- batchParallel is useful for automating multiple samples but can mask QC issues; review results carefully.
...
### annotate peaks with annotatePeaks using hg38 RefSeq annotation
**Args:** `annotatePeaks peaks.txt hg38 -gtf genes.gtf > annotated_peaks.txt`
**Explanation:** maps each peak to promoter/exon/intron/intergenic regions; -gtf overrides default HOMER annotation

### run de novo and known motif analysis with findMotifsGenome
**Args:** `findMotifsGenome peaks.txt hg38 motif_output/ -size 200 -mask -p 8`
...
### convert HOMER peak file to BED with pos2bed
**Args:** `pos2bed peaks.txt > peaks.bed`
...
**Args:** `findMotifsGenome peaks.txt hg38 motif_output/ -size 200 -bg background_peaks.txt -mask -p 8`
...
**Args:** `getDifferentialPeaksReplicates -t treatment_tag1/ treatment_tag2/ -c control_tag1/ control_tag2/ -genome hg38 -o diff_peaks.txt`
...
**Args:** `batchParallel makeTagDirectory tags -genome hg38 -checkGC -f sample1.bam sample2.bam sample3.bam`
**Explanation:** batchParallel runs makeTagDirectory in parallel on multiple BAM files; outputs to sample-specific directories

### find motifs of different lengths
**Args:** `findMotifsGenome peaks.txt hg38 motif_output/ -size 100 -len 8,10,12 -S 50 -mask -p 8`
...
**Args:** `analyzeRNA rna_tags/ mm10 -strand both -count exons -o rna_counts.txt`
