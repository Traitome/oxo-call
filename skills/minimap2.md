...
**Explanation:** minimap2 command; -ax map-ont Nanopore preset with SAM output; reference.fa reference genome; nanopore_reads.fastq.gz input reads; output piped to samtools sort
...
**Explanation:** minimap2 command; -ax map-hifi preset for PacBio HiFi/CCS reads with SAM output; reference.fa reference genome; hifi_reads.fastq.gz input reads; output piped to samtools sort
...
**Explanation:** minimap2 command; -ax splice enables spliced alignment for RNA-seq with SAM output; --junc-bed known_junctions.bed known splice junctions; reference.fa reference genome; rna_reads.fastq.gz input reads; output piped to samtools sort
...
**Explanation:** minimap2 command; -ax asm5 preset for assemblies with ≤5% divergence with SAM output; reference.fa reference genome; assembly.fa input assembly; output piped to samtools sort
...
**Explanation:** minimap2 command; -x map-ont Nanopore preset; -c includes CIGAR in PAF; reference.fa reference genome; reads.fastq.gz input reads; > aligned.paf output PAF

### compute all-vs-all overlaps for de novo ONT assembly
**Args:** `-x ava-ont reads.fastq.gz reads.fastq.gz | gzip > overlaps.paf.gz`
**Explanation:** minimap2 command; -x ava-ont preset for all-vs-all ONT overlaps; reads.fastq.gz input reads twice; output piped to gzip; > overlaps.paf.gz output

### build a reusable minimap2 index for repeated ONT alignments
**Args:** `-d reference_ont.mmi -x map-ont reference.fa`
**Explanation:** minimap2 command; -d reference_ont.mmi creates .mmi index file; -x map-ont preset; reference.fa input reference FASTA

### align with cs tag for variant calling
**Args:** `-ax map-hifi --cs reference.fa reads.fastq.gz | samtools sort -o aligned_cs.bam`
**Explanation:** minimap2 command; -ax map-hifi HiFi preset with SAM output; --cs outputs cs tag describing sequence differences; reference.fa reference genome; reads.fastq.gz input reads; output piped to samtools sort

### align with =/X CIGAR operators
**Args:** `-ax map-ont --eqx reference.fa reads.fastq.gz | samtools sort -o aligned_eqx.bam`
**Explanation:** minimap2 command; -ax map-ont Nanopore preset with SAM output; --eqx writes =/X instead of M in CIGAR; reference.fa reference genome; reads.fastq.gz input reads; output piped to samtools sort
...
**Explanation:** minimap2 command; -ax map-ont Nanopore preset with SAM output; --secondary=no outputs only primary alignments; reference.fa reference genome; reads.fastq.gz input reads; output piped to samtools sort

### align with soft clipping for SV calling
**Args:** `-ax map-ont -Y --MD reference.fa reads.fastq.gz | samtools sort -o sv_ready.bam`
**Explanation:** minimap2 command; -ax map-ont Nanopore preset with SAM output; -Y uses soft clipping for supplementary alignments; --MD adds MD tag; reference.fa reference genome; reads.fastq.gz input reads; output piped to samtools sort

### align short reads (Illumina)
**Args:** `-ax sr reference.fa reads_R1.fastq.gz reads_R2.fastq.gz | samtools sort -o short_aligned.bam`
**Explanation:** minimap2 command; -ax sr preset for short reads with SAM output; reference.fa reference genome; reads_R1.fastq.gz reads_R2.fastq.gz paired-end input; output piped to samtools sort
...
**Explanation:** minimap2 command; -ax map-ont Nanopore preset with SAM output; -k 17 increases k-mer size; -w 10 sets window size; reference.fa reference genome; reads.fastq.gz input reads; output piped to samtools sort

### output only mapped reads
**Args:** `-ax map-ont --sam-hit-only reference.fa reads.fastq.gz | samtools sort -o mapped_only.bam`
**Explanation:** minimap2 command; -ax map-ont Nanopore preset with SAM output; --sam-hit-only excludes unmapped reads; reference.fa reference genome; reads.fastq.gz input reads; output piped to samtools sort
