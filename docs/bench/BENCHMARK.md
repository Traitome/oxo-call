...
1. **Reference commands**: Each `### heading` / `**Args:**` / `**Explanation:**`
   ...
   `reference_commands.csv`.  Task descriptions are automatically **enriched
   with file/path tokens and package/module identifiers** extracted from the
   reference args so that the LLM has the precise filenames and package names
   needed to reproduce the reference command (e.g.
   `"sort a BAM file by genomic coordinates"` becomes
   `"sort a BAM file by genomic coordinates (sorted.bam, input.bam)"`,
   and `"install Bioconductor packages"` becomes
   `"install Bioconductor packages (DESeq2, edgeR)"`).
   Skill file headings also include specific subcommand/script names (e.g.
   `"trim adapters with bbduk.sh"` rather than `"trim adapters"`) so that the
   enrichment step only needs to add filenames that are not already present.
