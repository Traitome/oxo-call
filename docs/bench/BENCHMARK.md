...
correct command-line invocations across 138 bioinformatics tools spanning 40
analytical domains.
...
| Reference scenarios | 1,380 (10 per tool) |
| Natural-language descriptions | 6,900 (5 phrasings per scenario) |
| Ablation scenarios | 5 (bare, prompt, skill, doc, full) |
| Evaluation task catalog | 74 curated LLM evaluation tasks |
| Simulated experimental scenarios | 9 omics assay types |
...
The benchmark dataset is derived programmatically from the 138 skill files in
...
1. **Reference commands**: Each `### heading` / `**Args:**` / `**Explanation:**`
   ...
   `<tool>_10`), yielding 1,380 reference commands stored in
...
2. **Usage descriptions**: 5 natural-language rephrasings per scenario are
   ...
   yielding 6,900 semantically diverse descriptions in `usage_descriptions.csv`.
...
The 138 tools span 40 bioinformatics and computational biology domains:
...
To contextualize the tool invocations, 9 representative omics experimental
...
The benchmark covers 138 tools across 40 domains, which represents broad but
